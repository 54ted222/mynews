...
- 一個生產級的 loop 由六個積木組成：**排程觸發、git worktree 隔離、skills（持久專案知識）、MCP 連接器、maker/checker subagent 分工、對話之外的外部狀態**。缺任何一塊，迴圈都會以可預測的方式退化。
- loop 和 cron 的差別不在「會不會定時跑」，而在**中間有沒有決策者**：cron 每次走同一條路徑執行固定腳本；loop 每一輪由模型讀取當前狀態、選擇行動、檢查結果、再決定繼續或停手。
- 社群已沉澱出一條參考流程：排程 → triage → 讀寫狀態 → 隔離 worktree → 實作 subagent → 驗證 subagent → MCP/Git 落地 → human gate——安全路徑自動 commit，風險路徑附完整脈絡升級給人。
...
**6. Memory／State（外部狀態脊柱）**——解決「模型每輪之間什麼都不記得」。context window 不是記憶體，是暫存緩衝；真正的記憶必須活在模型之外——磁碟上的 `STATE.md`、ticket 系統、或外部知識圖譜（如 Zep Graphiti）。Ralph 系迴圈的核心架構正是如此：每一輪用全新 context 重啟 agent，狀態靠檔案往前傳，一次解掉 context 耗盡、狀態持久化、任務接續三個問題。缺了這塊，迴圈沒有連貫性：同一件事重做三次、做過的決策被推翻、「下一步是什麼」每輪重新發明。六塊裡面，這是最常被新手省略、也最快讓迴圈崩潰的一塊。
