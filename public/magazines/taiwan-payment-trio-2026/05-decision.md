...
- **Indie SaaS[^indie] / 訂閱型產品**：主選藍新（卡號綁定/定期定額成熟），次選綠界 AllInOne[^allinone] 定期定額。理由：藍新的 token 化與排程回呼較完整；綠界對 indie 開發者門檻更低[^3][^4]。
...
- **非營利捐款**：主選綠界或藍新（皆有定期定額 + netiCRM[^neticrm] 現成模組），次選 TapPay[^tappay]。理由：定期定額的卡號續期、暫停、改額功能成熟，捐款人不會因為卡片到期斷捐[^8]。
- **跨境試水**：紅綠藍三家都不是最佳解，請改用 Stripe[^stripe] / Paddle[^paddle] / Lemon Squeezy[^lemonsqueezy]（簡述於後，細節見 Vol.2 第三篇）[^9]。
...
- 訂閱時做 **dunning[^dunning]**、smart retry、card updater

請直接用 Stripe（DIY 稅務）或 Paddle / Lemon Squeezy（**Merchant of Record[^mor]**，他們替你處理稅）[^9][^18]。2026 年 2 月 Stripe 推出 Managed Payments、4 月 Lemon Squeezy 公告與 Stripe 雙線並行；**MRR[^mrr]** 在 $100K 美金以下，Lemon Squeezy 仍是 indie 最快上線的選項[^18][^19]。
...
[^indie]: Indie SaaS 指由一兩位獨立開發者或小團隊經營的軟體即服務產品，特色是工程資源有限、客戶量從零起跳、靠訂閱費維生。對金流的需求集中在快速上線、定期定額穩定扣款、發票自動化。
[^allinone]: AllInOne 是綠界 ECPay 的整合金流產品名，把信用卡、ATM、超商代碼、超商條碼、Apple Pay、Google Pay 等所有支付方式包在同一支 API。商家只需要串一次，由綠界決定要把使用者導向哪一個收款管道。
[^neticrm]: netiCRM 是台灣 NPO 圈最常用的開源 CRM 系統，以 CiviCRM 為核心二次開發，提供捐款管理、定期定額、活動報名、財報對帳等功能，並原生整合藍新、綠界、TapPay、MyPay 等台灣金流。
[^tappay]: TapPay 是台灣本土支付科技公司 cherri tech 的金流品牌，以「接近 Stripe 的開發者體驗」與 SDK 品質著稱，主要走中大型 SaaS 與企業客戶，月費門檻較高。國泰金控 2020 年策略入股後生態加速擴張。
[^stripe]: Stripe 是美國金流獨角獸，2010 年成立，全球 SaaS 訂閱業實質標配。提供完整 API、定期定額、發票、Tax、Connect 多人分潤等模組，但台灣商家身分要透過 Stripe Atlas 或海外公司架構才能直接收款。
[^paddle]: Paddle 是英國 SaaS 金流，採 **Merchant of Record** 模式——對外是 Paddle 賣給消費者、再分潤給商家，所有跨境稅務（VAT / Sales Tax / GST）由 Paddle 負責申報與繳納。適合不想自己處理多國稅務的 indie SaaS。
[^lemonsqueezy]: Lemon Squeezy 是 2021 年成立的美國 SaaS 金流，同樣走 Merchant of Record 模式、對 indie hacker 與小型 SaaS 特別友善。2024 年被 Stripe 收購，2026 年宣布與 Stripe Managed Payments 雙線並行。
[^dunning]: Dunning 指訂閱扣款失敗後的催收與重試流程，包括自動重試（smart retry）、寄信通知用戶更新卡號、暫停服務的階段性策略。優秀的 dunning 工具能把訂閱續扣失敗率從 8–10% 降到 3% 以下，對 SaaS 終身價值影響極大。
[^mor]: MoR（Merchant of Record，登記商家）指實際對消費者銷售並開立發票、繳稅的法人主體。SaaS 出海時，用 Paddle / Lemon Squeezy 當 MoR 可以把多國 VAT、Sales Tax 的法遵全包出去；自己當 MoR 則要逐國登記稅號。
[^mrr]: MRR（Monthly Recurring Revenue，月度經常性收入）是 SaaS 業最常用的營收指標，把所有訂閱合約的金額換算成「每月固定可收金額」。$100K MRR 是業界常用的 indie SaaS / growth-stage 分水嶺。
