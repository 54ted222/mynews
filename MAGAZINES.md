...
> 適用於主 agent 規劃主題頁，也適用於每個 subagent 寫自己那篇。
...
## 研究要求（先做再寫，不可省略）
- 開寫前用 WebSearch 做 3–5 次查詢、對關鍵文章用 WebFetch 讀全文
- 所有事實（公司、產品、數字、事件）都必須有可追溯 URL
- 可信來源等級：官方一手 > 第一手討論（HN / Reddit / PH）> 高訊號分析
  > 新聞媒體；避免內容農場與 AI 聚合站
- **嚴禁憑記憶寫**——訓練資料截止後的動態、具體數字、新產品名稱，
  一律現搜現查；不確定就標「估算」或略過
- 若關鍵面向查不到可信資料，回報「此角度資料不足，建議 X」，
  不要用推測填空
...
## 主 agent 已蒐集的 research notes（可直接引用，也請自行補充驗證）
- <fact 1> — <url> (<日期>)
- <fact 2> — <url> (<日期>)
- ...
...
- 回報 200 字內：寫了什麼、引用了哪些來源、有哪些判斷上的取捨
...
5. （可選）啟 dev server 用 Playwright 掃一次
   ...
   三層頁面，確認渲染正常
6. **先等使用者 review**，同意後再 commit / push：
   ```bash
