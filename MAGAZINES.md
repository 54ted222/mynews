...
主題的內容策劃與撰寫**，產出一個放在 `public/magazines/<topic-slug>/` 下的
...
## 寫作規格
- 字數：最少 1000–1800 字（繁體中文），不包含 quote block、bullet、圖片說明等非連續文本
- 結構：
  1. 正文第一行放 `# <title>` H1
...
階段二所有文章驗收通過後，**每篇文章再開一個 subagent** 做兩件事：

1. **在正文裡補上「名詞／服務／公司」的註解**（用 GFM footnote）
2. **另外寫一份語音逐字稿 sidecar**（`<slug>.transcript.md`）

可平行派出（一次多個 Agent 工具呼叫，一篇一個）。主 agent 一樣不要
自己寫，只派工 + 驗收。**`_index.md` 也要走這階段**：只需要逐字稿
（`_index.transcript.md`），名詞註解可選。
...
## 你的任務
為一篇已寫好的雜誌文章補兩件事：
1. 在原檔內加上名詞／服務／公司／縮寫的 GFM footnote 註解
2. 另寫一份語音逐字稿 sidecar：`<原檔路徑>.transcript.md`

寫完即結束。不要動正文既有論點、結構、數字、來源段。

## 原檔路徑（要就地修改）
public/magazines/<topic-slug>/<slug>.md

（若是主題頁，原檔為 public/magazines/<topic-slug>/_index.md；
  逐字稿 sidecar 為 public/magazines/<topic-slug>/_index.transcript.md；
  主題頁的名詞註解可選，逐字稿必做。）
...
### 工作 A：在正文加 GFM footnote 註解

- 挑 **3–8 個**讀者可能不熟的**專有名詞、服務、公司、縮寫、技術概念**
  加註解。選取標準是「外行讀者剛接觸會卡住的詞」，太常識的
  （HTTP、API 之類）略過
- 寫法：原文內用 `術語[^slug]` 標註；文末用 `[^slug]: 定義（40–120 字）`
  放定義。slug 用 kebab-case 英數
- 定義要中性、可獨立閱讀、**不重複正文已有的論點**——補充的是「這個
  詞是什麼 / 這家公司做什麼 / 為什麼值得看」
- 定義**不計入正文字數限制**
- 同一術語第一次出現時標註即可，不用每次都標
- 既有的 `---` 分隔線 + 來源段保留；footnote 定義放在**來源段之前**
  （與正文之間用空白行分隔）

### 工作 B：寫語音逐字稿 sidecar

- 路徑：`<原檔路徑>.transcript.md`
...
## 交付
- 用 Edit 工具修改原檔（加 footnote）
- 用 Write 工具產出 sidecar
- 回報 150 字內：加了哪些 footnote 術語、逐字稿抓了哪條主線
- 不要執行 git、不要動其他檔案、不要再 WebSearch（研究階段已結束）
```

#### 驗收要點（階段三）

- footnote 數量合理（正文 3–8 個、主題頁 0–4 個），選的是讀者可能
  不熟的專有名詞／服務／公司，不是常識
- 每條 footnote 定義 40–120 字、獨立可讀、不重述正文論點
- `<slug>.transcript.md` 存在、開頭口語開場、**沒有** frontmatter 與
  markdown heading
- 逐字稿把表格／bullet 口語化，不是直接念 `| 欄 1 | 欄 2 |`
- `_index.transcript.md` 也存在
